(function () {
    ...
    this.VisanetCheckout.onProd = (host(scriptTag.src).indexOf('static-content.vnforapps.com') !== -1);
    this.VisanetCheckout.host = (this.VisanetCheckout.onProd) ?
     'https://static-content' + (this.VisanetCheckout.isStatic ? '-nlb' : '') + '.vnforapps.com/vToken' : 'https://static-content-qas' +  (this.VisanetCheckout.isStatic ? '-nlb' : '') + '.vnforapps.com/vTokenSandbox';
    //this.VisanetCheckout.host = (this.VisanetCheckout.onProd) ? 'http://localhost:8080/visanet' : 'http://localhost:8080/visanet';
...
(function (document) {
    ...
    this.VisanetCheckout.validate = {
        ...
        preprocess: function (params) {
            ...
            if (params.channel) {
                params.channel = this.inValues(params.channel.toLowerCase(), 'web', 'mobile', 'callcenter', 'recurrent', 'pasarela', 'paycard') ? params.channel.toLowerCase() : 'web';
            }
        ...
        combinate: function (params, config) {
            if (params && config) {
                ...
                config.closeredirect = (params.closeredirect) ?  params.closeredirect.trim().toLowerCase() : (config.closeredirect ? config.closeredirect.trim().toLowerCase() : 'false');
            }
